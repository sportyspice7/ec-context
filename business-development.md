# Business Development — ExploreClairemont

## Overview
Tanja sells hyperlocal advertising and partnership packages to local businesses in the Clairemont area. The pitch is always community-first — businesses partner with ExploreClairemont because our audience trusts us and we live here too.

## Key Stats (as of early 2026)
- **Newsletter:** 2,000 subscribers, 60%+ open rate (well above industry average)
- **Website:** 3,000 monthly visitors
- **Instagram:** 1,685 followers, 103% growth in 30 days
- **Social reach:** 150,000+ avg monthly local views, 13.6% avg engagement rate

## Audience Demographics
- 59% have an undergraduate degree or higher
- 70% are married / 30% single
- 68% earn $100k+
- 82% are homeowners
- 46% male / 54% female

## What the Audience Wants (2026 Survey)
Top requested business categories for deals:
Restaurants, Breweries, Coffee/Tea, Home Services, Pet-Related, Salons, Cosmetic Spas, Wellness, Auto, Fitness, Yard/Landscape, Cleaning Services, Dental

Top requested discount types:
Vouchers (e.g. $40 for $20), Coupons (e.g. free appetizer), Percentage Off, BOGOs, First Session Free

---

## What We Offer Partners

### 1. Newsletter Sponsorships
- Weekly Fast Times newsletter reaches 2,000 locals
- 60%+ open rate
- Placements: top-of-email logo, Local Spotlight feature, in-content display ad

### 2. Display Advertising
- Banner and display ads on the website
- Custom creative — Tanja can create or business provides artwork
- Strategic placement alongside complementary content
- Performance analytics included

### 3. Social Media
- Reels, posts, stories, carousels
- Tanja can create and film content or use business-provided content
- Performance analytics per campaign

### 4. Community Promotions
- Explorer Exclusives — digital coupon/voucher system (3 tiers: Basic, Standard, Premium)
- Contests and giveaways
- Time-limited programs (e.g. Coffee Crawl)
- Backend dashboard with analytics

### 5. Content Collaboration
- Feature stories on website and in newsletter
- Sponsored content: local guides, round-ups, lead magnets
- Evergreen content exposure

### 6. Business Directory
- Basic listings always free
- Featured listings: appear first in searches, direct inquiry form, homepage rotation, analytics

### 7. Event Directory
- Basic listings free
- Featured listings: banner, inquiry form, newsletter inclusion

---

## Partnership Approach
- **No one-size-fits-all packages** — Tanja creates customized proposals based on business goals
- Proposals are sent as PDFs via Google Drive
- Introductory/trial periods offered before committing to long-term partnerships
- Exclusive partnerships available for certain categories (e.g. one real estate partner)
- **Tanja is in early growth mode (launched July 2025)** — she intentionally prices low to attract first advertisers, build case studies, and prove the platform's value
- The strategy is: low barrier to entry now → demonstrate results → raise rates as the platform grows

## Add-On Pricing Examples
- Social media carousel post: $125
- Social media reel: $225
- Feature article (newsletter + website permanent): $1,500

---

## Sales Materials
- **EC Media Kit** — 9-page PDF covering platform overview, audience stats, offerings
- **EC Marketing Flyers** — 12-page PDF with individual service one-pagers
- **Proposals** — customized per business, stored in Google Drive EC Marketing Kit folder
- Contact for partnerships: partner@exploreclairemont.com
- Phone: 760.522.6120

---

## Tanja's Background (relevant to sales)
- Journalism degree
- Career in public relations, marketing, and sales
- Clairemont native — authenticity and community trust are core to the pitch
- Personally writes all content, manages all partnerships

## Proposal Examples

### Coastline Real Estate Group (March 2026)
- Contact: Shaylie Gilliland (shaylie@coastlinesandiego.com)
- Exclusive real estate partner opportunity
- 3-month intro: $1,800 ($600/month) — intentionally low to attract first partner
- Tanja met with Shaylie in person before sending proposal
- Proposal sent March 10, follow-up March 17, second follow-up March 20
- Status as of March 2026: awaiting response — Shaylie acknowledged receipt but hasn't decided
- Tanja's follow-up style: warm, low pressure, gives them an out ("completely understand if it's not an option right now")

### Little Backstube (March 2026)
- Contact: Renate Neira (littlebackstube@gmail.com)
- German bakery, pickup-based business (locations: UTC/Swanson Pool area and Renate's house in Clairemont)
- Orders via hotplate.com/littlebackstube
- Special deal: $250 for 30-day Explorer Exclusives beta test (15% off voucher) + feature article in newsletter + promotion on socials
- Tanja actively helped with redemption process — worked late, troubleshot merchant dashboard issues with Renate directly
- Readers responded well — community engaged with the offer and article
- This is a great example of Tanja going above and beyond for a partner

### Indya Restaurant (January 2026)
- Contact: Gubi Khindra (eatindya@gmail.com)
- Featured story written by Tanja, included in Fast Times newsletter
- Result: owner personally thanked Tanja, reported guests coming in specifically because of the article
- Demonstrates the real impact of content collaboration on local businesses
- Received multiple 5-star reviews through Explorer Exclusives system after the feature

---

## Notes for Claude
- When helping write proposals, match the warm but professional tone of existing proposals
- Always lead with community trust and hyperlocal focus — that's the differentiator
- Proposals are customized — ask Tanja about the specific business and their goals before drafting
- Never suggest one-size-fits-all packages — Tanja always tailors her offerings
- Partner email: partner@exploreclairemont.com (different from tanja@exploreclairemont.com)
